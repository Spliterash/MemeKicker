...
public abstract class FixedEventsHandler {

    private static final String OVERRIDING_ERR = "Method of handling event is not overridden";

    private static final Logger LOG = LoggerFactory.getLogger(FixedEventsHandler.class);

    protected final Gson gson = new Gson();

    private <T> T designateObject(JsonObject object, FixedEvents type) {
    ...
    protected void messageReply(Integer groupId, MessageObject message) {
        LOG.error(OVERRIDING_ERR);
    }

    protected void messageEdit(Integer groupId, MessageObject message) {
        LOG.error(OVERRIDING_ERR);
    }
    ...
    protected String parse(FixedCallbackMessage message) {
        switch (message.getType()) {
            ...
            case PHOTO_NEW -> photoNew(message.getGroupId(), designateObject(message.getObject(), message.getType()));
            ...
            case AUDIO_NEW -> audioNew(message.getGroupId(), designateObject(message.getObject(), message.getType()));
            case VIDEO_NEW -> videoNew(message.getGroupId(), designateObject(message.getObject(), message.getType()));
            ...
            case GROUP_JOIN -> groupJoin(message.getGroupId(), designateObject(message.getObject(), message.getType()));
            ...
            case USER_BLOCK -> userBlock(message.getGroupId(), designateObject(message.getObject(), message.getType()));
            case USER_UNBLOCK ->
